...
public class wordladder {
    ...
    public Stack<String>wl_stack(){
        Set<String>used_words=new HashSet<>();        // store used words
        Queue<Stack<String>>wlstack_queue = new LinkedBlockingQueue<>();  //store temp wl stack
        Stack<String>single_wl = new Stack<>();    //stack uesd to store wl
        ...
        {
            temp1 = (Stack<String>)wlstack_queue.peek().clone();
            wlstack_queue.poll();
        ...
        Stack<String>no_wl=new Stack<>();
        no_wl.push("No word ladder.");
        return no_wl;
    }
    public void print_wl(Stack<String>wl){
        String temp;
        String output="";
        int size=wl.size();
